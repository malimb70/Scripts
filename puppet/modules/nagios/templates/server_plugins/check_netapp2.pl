...
#
# Updated to fix 2T Volume issues with unsigned integers
#
# 2010.05.04 Version 1.1
#
#############################################################
#
...
#
# Updated by Simon McGregor
# 2011.05.03
# 
# Issues with DISKUSED64 not being available as an Option. This is needed
# for Volumes above 2T.
#############################################################
#
# $Id: check_netapp,v 1.2 2011/05/03 16:03:16 root Exp root $
...
# SJM - Modification to get 64 bit vlaues to correct 2T limit issue
my $snmpfilesysdfTabledfEntry64highdfKBytesTotal = '.1.3.6.1.4.1.789.1.5.4.1.14';
my $snmpfilesysdfTabledfEntry64lowdfKBytesTotal = '.1.3.6.1.4.1.789.1.5.4.1.15';
my $snmpfilesysdfTabledfEntry64highdfKBytesUsed = '.1.3.6.1.4.1.789.1.5.4.1.16';
my $snmpfilesysdfTabledfEntry64lowdfKBytesUsed = '.1.3.6.1.4.1.789.1.5.4.1.17';
my $snmpfilesysdfTabledfEntry64highdfKBytesAvail = '.1.3.6.1.4.1.789.1.5.4.1.18';
my $snmpfilesysdfTabledfEntry64lowdfKBytesAvail = '.1.3.6.1.4.1.789.1.5.4.1.19';
# End of Mods
...
# SJM 64bit disk modification
} elsif ( $variable eq 'DISKUSED64' ) {
        $state = 'OK';
        foreach my $key ( keys %{$snmpresponse{$snmpfilesysdfTabledfEntrydfFileSys}} ) {
                if ( defined $volume ) {
                        if ( $snmpresponse{$snmpfilesysdfTabledfEntrydfFileSys}{$key} eq $volume ) {
                                my $volume = $snmpresponse{$snmpfilesysdfTabledfEntrydfFileSys}{$key};
                                my $used64high = $snmpresponse{$snmpfilesysdfTabledfEntry64highdfKBytesUsed}{$key};
                                my $used64low = $snmpresponse{$snmpfilesysdfTabledfEntry64lowdfKBytesUsed}{$key};
				my $used = $used64low;
				my $power32 = 2 ** 32;
				if ( $used64low >= 0 ) {
					$used = ($used64high * $power32) + $used;
					}
				if ( $used64low < 0 ) {
					$used = (($used64high + 1) * $power32) + $used;
					}
                                my $total64high = $snmpresponse{$snmpfilesysdfTabledfEntry64highdfKBytesTotal}{$key};
                                my $total64low = $snmpresponse{$snmpfilesysdfTabledfEntry64lowdfKBytesTotal}{$key};
				my $total = $total64low;
				if ( $total64low >= 0 ) {
					$total = ($total64high * $power32) + $total;
					}
				if ( $total64low < 0 ) {
					$total = (($total64high + 1) * $power32) + $total;
					}
                                my $avail64high = $snmpresponse{$snmpfilesysdfTabledfEntry64highdfKBytesAvail}{$key};
                                my $avail64low = $snmpresponse{$snmpfilesysdfTabledfEntry64lowdfKBytesAvail}{$key};
				my $avail = $avail64low;
				if ( $avail64low >= 0 ) {
					$avail = ($avail64high * $power32) + $avail;
					}
				if ( $avail64low < 0 ) {
					$avail = (($avail64high + 1) * $power32) + $avail;
					}
                                my $percent = $snmpresponse{$snmpfilesysdfTabledfEntrydfPercentKBytesCapacity}{$key};
                                $answer = sprintf("%s - total: %d Kb - used %d Kb (%d%%) - free: %d Kb",$volume,$total,$used,$percent,$avail);
                                $perfdata = sprintf("NetApp %s Used Space=%dKB;%d;%d;0;%d",$volume,$used,$total*$warning/100,$total*$critical/100,$total);
...
sub print_help () {
        ...
        print "\t\tDISKUSED - disk space used less that 2T (depreciated)\n";
        print "\t\tDISKUSED64 - disk space used greater than 2T\n";
