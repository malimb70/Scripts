...
$mail_cmd = q{/bin/mail}; 
$subject="carepages_bj_job_stats.pl $target_host";	# default email subject line
$HOME=$ENV{HOME};
my $state = "";
...
if (!$dbh) {
  $state = "CRITICAL";
  print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : cannot connect to $target_host\n";
  exit $ERRORS{"$state"};
  }
...
if (!$sth) {
  $state = "CRITICAL";
  print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : The statement prepare failed on $target_host\n";
  exit $ERRORS{"$state"};
  }
...
if (!$res) {
  $state = "CRITICAL";
  print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : The statement execution failed on $target_host\n";
  exit $ERRORS{"$state"};
  }
...
if (my @dat=$sth->fetchrow_array()) {
	...
	$db[$x]=$dat[0];
        ...
        if ($current_count > $error_thresh )  {
          $state = "CRITICAL";
          print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : The number of rows on the $table_name on $target_host is $current_count which is higher than $error_thresh\n";
          exit $ERRORS{"$state"};
          # This should generate an error
          }
        elsif  ( ( $current_count < $error_thresh ) &&  ($current_count > $warning_thresh) ) {
          $state = "WARNING";
          print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : The number of rows on the $table_name on $target_host is $current_count which is higher than $warning_thresh \n";
          exit $ERRORS{"$state"};
          }
        else {
          $state = "OK";
          print STDOUT "CAREPAGES BJ JOB STATS : " . $state . " - : The number of rows on the $table_name on $target_host is $current_count\n";
          exit $ERRORS{"$state"};
          }
