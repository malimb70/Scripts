...
if (@o_criticalHttpCodes or @o_criticalExpressions) {
  ...
  foreach $ccode (@o_criticalHttpCodes) {
    if ($code =~ m/$ccode/) {
      ...
      #print "Status: Critical.  " . $o_host . $o_url . " Matching critical HTTP Code $code\n";
      print "Status: Critical.  " . $url . " Matching critical HTTP Code $code\n";
      my $state = "CRITICAL";
      exit $ERRORS{$state};
    }
  }
  ...
  foreach $cexpr (@o_criticalExpressions) {
    if ($content =~m/$cexpr/) {
      print "Status: Critical.  " . $url . " Matching critical regular expression\n";
      my $state = "CRITICAL";
      exit $ERRORS{$state};
    }
  }
...
if (@o_warningHttpCodes or @o_warningExpressions) {
  ...
  foreach $ccode (@o_warningHttpCodes) {
    if ($code =~ m/$ccode/) {
      # print $response->status_line, "\n";
      print "Status: Warning.  " . $url . "  HTTP Code $code\n";
      my $state = "WARNING";
      exit $ERRORS{$state};
    }
  }
  ...
  foreach $cexpr (@o_warningExpressions) {
    if ($content =~m/$cexpr/) {
      print "Status: Warning.  " . $url . "  Matching warning regular expression\n";
      my $state = "WARNING";
      exit $ERRORS{$state};
    }
  }
  ...
  # no critical condition was supplied by user
  if ($code >= 300) {
    print "Status: WARNING " . $url . "  (" . $response->status_line . ")\n";
    my $state = "WARNING";
    exit $ERRORS{$state};
  }
}
