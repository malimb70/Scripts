...
my %oskeys=(
        'centos6-64bit' =>      '1-centos-6-x64-key',
...
my %URLs=(
        ...
        'rhn-org' => 'http://usnjlswalk01.waterfrontmedia.net/pub/rhn-org-trusted-ssl-cert-1.0-1.noarch.rpm',
);
...
$cmd="sed -i 's/https/http/g' /etc/yum.repos.d/epel.repo";
$pid = open3($writer, $reader, $err, $cmd);
$retCode = <$reader>;
...
if ($retCode eq "")
{
        $cmd="yum install rhn-client-tools rhn-check rhn-setup rhnsd m2crypto yum-rhn-plugin osad rhncfg rhncfg-actions rhncfg-client -y";
        $pid = open3($writer, $reader, $err, $cmd);
        print <$reader>;
}
...
if ($retCode =~ m/Unable/)
{
	$cmd = "rhnreg_ks --serverUrl=https://usnjlswalk01.waterfrontmedia.net/XMLRPC --sslCACert=/usr/share/rhn/RHN-ORG-TRUSTED-SSL-CERT --activationkey=".$oskeys{$iver};
	$pid = open3($writer, $reader, $err, $cmd);
	my $str = <$reader>;
	if ($str eq "")
	{
